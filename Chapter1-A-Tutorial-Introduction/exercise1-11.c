...
|are most likely to uncover bugs if thera are any?                             |
*/
...
0 0 0 (zero newlines; zero words; zero characters).
    Then try a one-character word. The output should be:
1 1 2 (one newline; one word; two characters - a letter followed by a newline).
    Then try a two-character word. The output should be:
1 1 3 (one newline; one word; three characters - two characters followed by a   
newline).
